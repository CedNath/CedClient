...
/**
 * A single-line text field with caret, click-drag selection, double/triple
 * click word/line select, undo/redo history, and standard Ctrl shortcuts.
 * Ported 1:1 from Odin's utils/ui/TextInputHandler.kt — SearchBar (Phase 4)
 * is its only user right now, but it's a generic field, not clickgui-specific.
 *
 * NOTE: uses `input.key` as a property (KeyEvent), matching how Odin's real,
 * compiling code reads it. features/settings/KeybindSetting.kt in this
 * project instead calls `input.key()` as a function and flags that as an
 * unconfirmed guess — worth reconciling the two against your decompiled
 * KeyEvent source once you've got it open, since they can't both be right.
 */
class TextInputHandler(
    ...
    fun mouseClicked(mouseX: Float, mouseY: Float, click: MouseButtonEvent): Boolean {
        ...
        when (clickCount) {
            1 -> {
                updateCaretFromMouse(mouseX); clearSelection()
            }

            2 -> selectWord()
            3 -> {
                selectAll(); clickCount = 0
            }
        }
        return true
    }
    ...
    private fun handleCtrlShortcut(input: KeyEvent): Boolean {
        ...
        return when (input.key) {
            GLFW.GLFW_KEY_V -> {
                insert(mc.keyboardHandler.clipboard); true
            }

            GLFW.GLFW_KEY_C -> if (hasSelection()) {
                mc.keyboardHandler.clipboard = selectedText(); true
            } else false

            GLFW.GLFW_KEY_X -> if (hasSelection()) {
                mc.keyboardHandler.clipboard = selectedText(); deleteSelection(); true
            } else false
