...
/**
 * HSB-backed color, ported from Odin. Stores hue/saturation/brightness/alpha
 * and lazily recomputes the packed [rgba] int only when one of those changes
 * — this is what makes brighter()/darker() (which nudge brightness) cheap
 * and keeps hue stable through repeated adjustments, unlike mutating a raw
 * RGB int directly.
 */
...
class Color(hue: Float, saturation: Float, brightness: Float, alpha: Float = 1f) {
    ...
    /**
     * Used to tell [rgba] to recompute when the HSBA values are changed.
     */
    @Transient
    private var needsUpdate = true

    /**
     * Packed RGBA value. Recomputed from HSBA on first read after a change.
     */
    var rgba: Int = 0
        get() {
            if (needsUpdate) {
    ...
    /**
     * Whether this color isn't visible at all — use to skip a draw call
     * outright rather than rendering a fully transparent shape.
     */
