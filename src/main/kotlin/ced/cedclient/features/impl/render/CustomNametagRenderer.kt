 ...
 *    PlayerRendererMixin uses for its hardcoded scale target) gets
 *    CustomNametag.HARDCODED_TAG, regardless of CustomNametag's own module
 *    toggle -- gated instead by HardcodedCosmetics.isEnabled, so viewers
 *    can turn this cosmetic off on their own client.
...
object CustomNametagRenderer {
    ...
    fun register() {
        LevelRenderEvents.AFTER_SOLID_FEATURES.register(LevelRenderEvents.AfterSolidFeatures { context ->
            ...
            val camera = context.gameRenderer().mainCamera() // 26.2: renamed from getMainCamera(), and dropped the "get" prefix so Kotlin can't sugar it into property syntax -- must call with parens
    ...
    private fun submitTag(
        ...
        // 26.2: submitNameTag dropped the maxDistanceSq (squared-distance-to-camera)
        // parameter entirely -- it's just (poseStack, attachment, offset, name,
        // seeThrough, lightCoords, cameraRenderState) now, 7 args, no distance cap.
